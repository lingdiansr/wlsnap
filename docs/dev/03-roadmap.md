...
|------|-----------|---------|-------------|
| **v0.1.0** | M1 | CLI 截图 + 保存/剪贴板/stdout/exec | 命令行用户快速截图，无 GUI 编辑 |
...
**目标**: 在 Hyprland / Sway / Niri 上，用户通过 CLI 执行 `--full`、`--area`（硬编码区域）、`--screen`、`--full-all` 后，截图可直接保存到文件、复制到剪贴板、输出到 stdout 或通过 `--exec` 调用外部程序。无 GUI 编辑器，无 Pin，无长截图。
...
| T9 | 事件桥接：tokio runtime + `mpsc::unbounded_channel` + `ctx.request_repaint()` | T4 + T6 + T8 | 中 | 后台截图完成后，UI线程能收到事件并执行保存/剪贴板/stdout/exec |
